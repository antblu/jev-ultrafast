...
The demo can instead use the selected OpenAI-compatible LLM as its decision engine. It first receives the same structured page state and an enumerated operation/target vocabulary, then returns `screenshot_required` plus an offered operation and target when no image is needed. For visual information absent from that state, it may request the current inspector screenshot; the configured policy either sends it automatically or pauses for explicit approval. The visual response must then name an offered operation and target. Invalid, invented, or mismatched targets stop before execution. This mode cannot emit selectors, coordinates, or executable code, and screenshot data is redacted from traces and logs.
...
The model sees visible text. Background focus emulation keeps animation frames running in the owned tab. Screenshots are optional and disabled in library calls by default; `screenshots=True` or `record_dir=...` enables them. The inspector enables them explicitly and may send the current image only through the requested-and-permitted Selected LLM flow above. A continuous screencast can record a run separately.
